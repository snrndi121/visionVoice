...
import com.kbas.FaceUtils.common.GraphicOverlay.Graphic;
...
//    }
    FirebaseVisionFaceContour contour_lower_lips_bottoms = face.getContour(FirebaseVisionFaceContour.LOWER_LIP_BOTTOM);
    for (com.google.firebase.ml.vision.common.FirebaseVisionPoint point : contour_lower_lips_bottoms.getPoints()) {
      float px = translateX(point.getX());
      float py = translateY(point.getY());
      canvas.drawCircle(px, py, FACE_POSITION_RADIUS, facePositionPaint);
    }
    FirebaseVisionFaceContour contour_lower_lips_top = face.getContour(FirebaseVisionFaceContour.LOWER_LIP_TOP);
    for (com.google.firebase.ml.vision.common.FirebaseVisionPoint point : contour_lower_lips_top.getPoints()) {
      float px = translateX(point.getX());
      float py = translateY(point.getY());
      canvas.drawCircle(px, py, FACE_POSITION_RADIUS, facePositionPaint);
    }
    FirebaseVisionFaceContour contour_uppper_lips_bottom = face.getContour(FirebaseVisionFaceContour.UPPER_LIP_BOTTOM);
    for (com.google.firebase.ml.vision.common.FirebaseVisionPoint point : contour_uppper_lips_bottom.getPoints()) {
      float px = translateX(point.getX());
      float py = translateY(point.getY());
      canvas.drawCircle(px, py, FACE_POSITION_RADIUS, facePositionPaint);
    }
    FirebaseVisionFaceContour contour_uppper_lips_top = face.getContour(FirebaseVisionFaceContour.UPPER_LIP_TOP);
    for (com.google.firebase.ml.vision.common.FirebaseVisionPoint point : contour_uppper_lips_top.getPoints()) {
      float px = translateX(point.getX());
      float py = translateY(point.getY());
      canvas.drawCircle(px, py, FACE_POSITION_RADIUS, facePositionPaint);
    }

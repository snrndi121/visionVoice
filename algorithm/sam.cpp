...
v_ptype res1, res2;
//func
char* generate_TESTCASE_N();
vector < SType > generate_Sn(const char *);
...
int main()
{
    char *src = generate_TESTCASE_N();
    vector < SType > dst = generate_Sn(src);
    for (int i = 0; i < dst.size(); ++i) { cout << dst[i].data << endl;}
    // find_KMP(src, dst);
...
char* generate_TESTCASE_N()
{
    ...
    char *input = new char[MAX_TEST_LEN];
    srand(time(NULL));
    for (int i = 0; i < MAX_TEST_LEN; ++i) {
      input[i] = 'A' + (rand() % HN_RANGE);
    }
    cout << ". done" << endl;
    return input;
}
vector < SType > generate_Sn(const char *_src)
{
    int cnt = 0;
    cout << "#generate_Hn ";
    vector < SType > res;
    ...
    for (size_t i = 0; i < MAX_HN_SIZE; ++i) {
        ...
        copy(_src+e_start, _src+e_start+e_sz, u);//[start, end]
        SType ele(e_sz, u);
        // cout << ele << endl;
        res.push_back(ele);
        prev_e_end = e_end;
